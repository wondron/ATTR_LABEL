...
切换只对当前浏览器会话生效：不同电脑或不同浏览器可以各自选择目录，互不影响。同一浏览器中的多个标签页会共享选择；其中一个标签页切换后，其他旧标签页会停止读写并提示刷新，防止写入错误目录。

网页切换不会修改 `current_label_data` 软链接或 `.env`，所以新浏览器会话或服务重启后会回到配置的初始目录。需要让新目录长期作为默认值时，请使用下面的命令行方式更新软链接。
...
点击左侧“数据筛查”，在弹窗中设置 8 项属性的筛选条件，再点击“应用筛选”。默认每项为“不限”；可选择“默认值”或填写指定值，食物名称还支持“包含关键词”。数值按相等匹配，多选属性命中任一勾选值即可，不同属性需同时满足。条件可与文件名搜索、标注状态筛选叠加。

筛选使用已保存的标注。没有 JSON 的图像以及缺失的属性按默认值匹配：食物名称为“无”，容器类型、附件类型、层位为 `["无"]`，数量、重量、型号和尺寸为未指定（`null`）。异常 JSON 不参与属性匹配，可通过“仅异常”状态单独查看。

列表和翻页只使用匹配结果。保存或实时更新后，若当前图像不再匹配且没有未保存修改，会自动选择剩余结果；没有结果时清空预览。未保存修改仍受保护，应用条件需要切换图像时会先提醒确认。

“重置条件”清空弹窗中的设置，点击“应用筛选”后生效；“取消”保留原筛选。左侧“清除”可直接清除属性条件，切换文件夹时也会重置条件。筛查不更改标注文件，下载和属性统计仍针对当前文件夹的全部有效标注。
